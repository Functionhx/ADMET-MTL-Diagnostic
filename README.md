...
| 2 | **The advantage is temperature-correctable** | Strict per-model temperature calibration (T fitted on each model's own 10% calibration partition, applied to its own test partition; 480 per-run fits, 110 reach the T = 5.0 upper bound) attenuates Δ to ≈ 0 under both protocols (Δ_cal = −0.003 / +0.003, Γ_cal = +0.006, p = 0.45); a log-space refit over T ∈ [0.05, 100] leaves the calibrated contrasts ≈ 0 (random −0.002, scaffold −0.001; Γ_cal,wide = +0.001) |
| 3 | **Not an optimization-budget effect** | Compute-matched STL (8× updates, 15 runs) is *worse* than standard STL (−0.096 random / −0.125 scaffold); MTL beats STL-8× by +0.396 |
| 4 | **Not task-specific label transfer** | Fully permuted auxiliary labels leave the gain intact under both protocols (random Δ = +0.296 vs +0.298; scaffold +0.441 vs +0.407); the protocol interaction **survives permutation** (Γ_permuted = +0.128 vs Γ_real = +0.095 at matched three-instance scale, numerically larger in two of three splits); pooled pretrain + STL fine-tune retains only +0.036 → the gain lives in *joint optimization of the shared representation* |
...
| 7 | **Protocol interaction is cross-architecture** | Positive direction reproduced in a matched-scale GIN sensitivity analysis (5 instances × 3 seeds): Γ = +0.026, split-instance bootstrap 95% CI [−0.006, +0.064] (directionally supportive rather than individually significant) |
