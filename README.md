...
| 4 | **Not task-specific label transfer** | Fully permuted auxiliary labels (target-specific, 9 runs) leave the gain intact (Δ = +0.300); pooled pretrain + STL fine-tune retains only +0.045 → the gain lives in *joint optimization of the shared representation* |
